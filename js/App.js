window.CONTRACT = {
  address: '0xD8D9300415e412fD51ff447D6cF7E0E13A51032d',
...
async function trashHash() {
  ...
  if (window.hashedfile) {
    await window.contract.methods
      ...
      .on('receipt', function (receipt) {
        $('#note').html(
          `<h5 class="text-info p-1 text-center">Document Trashd 😳</h5>`,
        )
...
async function trashExporter() {
  ...
  if (address) {
    ...
    try {
      await window.contract.methods
        ...
        .on('receipt', function (receipt) {
          ...
          console.log(receipt)
          $('#note').html(
            `<h5 class="text-info">Exporter Trashd Successfully 🙂</h5>`,
          )
        })
        .on('error', function (error) {
          console.log(error.message)
    ...
    $('#note').html(
      `<h5 class="text-center text-warning">You need to provide address to trash 👍</h5>`,
    )
  }
}
